...
from zillow_scrapper.supabase_client import (
    create_run,
    get_supabase_client,
    get_run_uuid,
    save_matched_result,
...
def run_scrape(
    *,
    ...
    console.print(f"[dim]Supabase URL: {'SET' if supabase_url else 'NOT SET'}[/dim]")
    console.print(f"[dim]Supabase Key: {'SET' if supabase_key else 'NOT SET'}[/dim]")
    ...
    if supabase_client:
        ...
        console.print("[yellow]Warning: Supabase client not initialized. Data will only be saved to JSON files.[/yellow]")
    ...
    try:
                    ...
                    try:
                        ...
                        if len(chosen_comps) >= 1:
                            ...
                            if supabase_client and run_uuid:
                                ...
                                console.print(f"[yellow]Warning: Supabase client not available, skipping save[/yellow]")
                            elif not run_uuid:
                                console.print(f"[yellow]Warning: Run UUID not available, skipping save[/yellow]")
        ...
        if supabase_client:
            update_run_status(supabase_client, run_id, "completed")
    except Exception as e:
